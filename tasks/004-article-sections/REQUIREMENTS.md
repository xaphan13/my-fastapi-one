...
Подпапки `content_art/` стали разделами блога: бэкенд хранит `section` в реестре, отдаёт
`GET /api/blog/sections` и фильтрацию `?section=`; React SPA получил левое меню разделов
(`SectionMenu`, маршрут `/section/:name`, активный пункт, «Все статьи»). Все 12 критериев
успеха подтверждены qa-прогоном (e2e/qa-success-criteria.txt), дефектов не найдено.

## Изменения

- `fastapi-application/md_articles/schema_art.py` — поле `section` (+автовычисление из
  `file_name` через `model_validator`), `_FIELDS_FOR_YAML`, рекурсивный `scan_content_art()`
  (относительные POSIX-пути), хелпер `get_section()`.
- `fastapi-application/md_articles/api_blog.py` — `SectionOut` и роут `GET /sections`
  (счётчик маршрутов 40 → 41); query-параметр `section` в `GET /articles`; `add_all` —
  title из `Path(...).stem` без префикса папки; `meta` — сохранение/вычисление `section`.
- `fastapi-application/md_articles/articles.yaml` — сериализованы `section: ''` для 5
  существующих корневых статей (обратная совместимость: YAML без поля тоже валиден).
- `frontend/src/types.ts` — интерфейс `Section`, `section?: string` в `Article`.
- `frontend/src/api/blog.ts` — `getSections()`, `getArticles(section?)`.
- `frontend/src/components/SectionMenu.tsx` — новый компонент левого меню (NavLink,
  «Все статьи» с `end`, тихий fallback при ошибке API).
- `frontend/src/App.tsx` — маршрут `/section/:name` (тот же `HomePage`).
- `frontend/src/components/Layout.tsx` — статические кнопки заменены на `<SectionMenu />`.
- `frontend/src/pages/HomePage.tsx` — чтение раздела из `useParams`, фильтрованный запрос,
  заголовок «Статьи раздела «{name}»», пустое состояние «В этом разделе пока нет статей.».
- `frontend/src/index.css` — `.menu-item.active` (подсветка активного пункта).

## Критерии успеха

| # | Критерий | Результат | Доказательство |
|---|---|---|---|
| 1 | Счётчик маршрутов = 41 | PASS | e2e/qa-success-criteria.txt |
| 2 | ruff чист по обоим файлам | PASS | e2e/qa-success-criteria.txt |
| 3 | `/api/blog/sections` отдаёт разделы | PASS | e2e/qa-success-criteria.txt |
| 4 | `?section=` фильтрует | PASS | e2e/qa-success-criteria.txt |
| 5 | Без параметра — все полные статьи | PASS | e2e/qa-success-criteria.txt |
| 6 | `add_all` регистрирует подпапки (section + title без префикса) | PASS | e2e/qa-success-criteria.txt |
| 7 | `art_manage` видит section и относительные пути | PASS | e2e/qa-success-criteria.txt |
| 8 | `npm run build` без ошибок | PASS | e2e/qa-success-criteria.txt, dev/phase03_raw.txt, dev/phase04_raw.txt |
| 9 | Меню: «Все статьи» + разделы | PASS | e2e/qa-success-criteria.txt (grep dist-бандла) |
| 10 | Клик по разделу фильтрует (`/section/<name>`) | PASS | e2e/qa-success-criteria.txt |
| 11 | «Все статьи» сбрасывает фильтр (`/`) | PASS | e2e/qa-success-criteria.txt |
| 12 | Активный пункт `.active` | PASS | e2e/qa-success-criteria.txt (.menu-item.active в CSS-бандле) |

Регресс соседних эндпоинтов (`/docs`, `/users/get_all_users`, `/orders/get_all_orders`,
`/api/v1/dep_examples/*`) — без нарушений (422 у двух — валидация их собственных
обязательных параметров, не связана с заданием).

## Дефекты

Не найдены — DEFECTS.md не создавался. Замечание о том, что запись из `add_all` становится
`complete` (и попадает в списки/разделы) только после заполнения author/lang через
`art_manage/meta` — исходное поведение порта flask-blog, не дефект задания.

## Adversarial-прогон

Отменён по решению пользователя (см. «Отступление от стандартного цикла» в спеке) —
ADVERSARIAL_REVIEW.md не создавался.

## Участники

- backend-dev: фаза 1 (schema_art.py), фаза 2 (api_blog.py + живой smoke с add_all/sections/фильтрами)
- frontend-dev: фаза 3 (типы, API-клиент, SectionMenu), фаза 4 (роутинг, Layout, HomePage, стили)
- qa: полный прогон 12 критериев успеха + регресс + очистка тестовых данных (e2e/qa-success-criteria.txt)
- adversary: не привлекался (прогон отменён пользователем)
- оркестратор: пре-флайт, ревью диффов всех фаз, контрольные прогоны checkpoint, архивирование
